...
- Added the first typed `runtime_healthcheck` control schema in `nettoolskit-core`, wired `ntk runtime healthcheck --json-output` through the shared contract, and extended the control-plane catalog/docs so machine-readable runtime inspection is no longer limited to doctor surfaces.
- Added the first typed `runtime_self_heal` control schema in `nettoolskit-core`, wired `ntk runtime self-heal --json-output` through the shared contract, and extended the control-plane catalog/docs so machine-readable runtime recovery is aligned with the same control-plane family as doctor and healthcheck.
- Added typed `local_context_query` and `local_memory_query` control schemas in `nettoolskit-core`, wired both runtime query JSON surfaces through the shared contract, and extended the control-plane catalog/docs so local recall now follows the same typed control-plane family as doctor, healthcheck, and self-heal.
- Added typed `ai_provider_profiles` and `ai_provider_profile` control schemas in `nettoolskit-core`, wired both `ntk ai profiles` JSON surfaces through the shared contract, and extended the control-plane catalog/docs so AI preset inventory now follows the same typed control-plane family as doctor and local recall.
...
- Added `ntk runtime clean-build-artifacts` as the native CLI surface for dry-run and forced removal of `.build`, `.deployment`, `bin`, and `obj` artifact directories through the repository-owned Rust maintenance boundary.
- Added measured byte inventory and reclaimed-byte reporting to `ntk runtime clean-build-artifacts`, and tightened artifact discovery so managed directories are counted once instead of recursively double-counting nested artifact paths.
- Archived the build-target cleanup and artifact-pruning workstream after baseline measurement, safe prune policy, native cleanup automation, and repository guidance all reached validated closeout.
...
- Changed `ntk runtime query-local-context-index --json-output` and `ntk runtime query-local-memory --json-output` so machine-readable recall now emits shared control schemas instead of ad-hoc command payloads.
- Changed `ntk ai profiles list --json-output` and `ntk ai profiles show --json-output` so machine-readable AI preset inspection now emits shared control schemas instead of serializing orchestrator structs directly.
...
- Closed the `plugin-skill-extensibility-and-runtime-governance` workstream after the canonical extension-governance catalog, architecture/sample docs, README coverage, and loading-boundary rules landed for agents, skills, hooks, provider prompts/projections, and future plugins.
- Closed the `memory-file-layering-and-operational-recall` workstream after the canonical operational-memory layering catalog, architecture/sample docs, README coverage, and governance rules landed for concise memory, topic memory, append-only notes, and retrieval alignment.
