...
- Expanded GitHub provider coverage under `definitions/providers/github/` so the repository-owned `.github/` projection now includes `ISSUE_TEMPLATE/`, reusable `templates/`, and the extra managed root files `PULL_REQUEST_TEMPLATE.md`, `dependabot.yml`, and `dependency-review-config.yml`; updated the GitHub renderer, provider projection catalog, validation gates, and documentation so the remaining GitHub-native exceptions are governance-only surfaces such as workflows, policies, schemas, runbooks, and governance catalogs.
