...
fn test_ensure_directory_creates_new_directory() {
    ...
    // Act
    let result = nettoolskit_manifest::execution::files::ensure_directory(
        &new_dir,
        false, // not dry-run
        &mut summary,
        "test directory",
...
fn test_execute_plan_creates_new_file() {
    ...
    // Act
    let result = nettoolskit_manifest::execution::files::execute_plan(changes, false, &mut summary);
...
fn test_execute_plan_dry_run_does_not_write() {
    ...
    // Act
    let result = nettoolskit_manifest::execution::files::execute_plan(changes, true, &mut summary);
