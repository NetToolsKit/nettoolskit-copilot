...
try {
    ...
    Assert-Equal -Actual @($allPreviewResult.steps).Count -Expected 8 -Message 'Install preview must plan the full onboarding steps for profile all.'
    ...
    Assert-Equal -Actual $allPreviewResult.steps[6].name -Expected 'Configure global Git aliases' -Message 'All-profile install preview must include global Git alias setup after local hooks.'
    Assert-Equal -Actual $allPreviewResult.steps[7].name -Expected 'Run repository healthcheck' -Message 'All-profile install preview must end with healthcheck.'
    ...
    Assert-Equal -Actual @($claudePreviewResult.steps).Count -Expected 2 -Message 'Claude-profile install preview must plan Claude skill sync plus healthcheck.'
    ...
    Assert-Equal -Actual $claudePreviewResult.steps[1].name -Expected 'Run repository healthcheck' -Message 'Claude-profile install preview must keep healthcheck.'
    ...
    Assert-Equal -Actual @($reducedPreviewResult.steps).Count -Expected 3 -Message 'Install preview must honor skip switches on top of profile all while keeping runtime-specific preferences and Claude skills sync.'
