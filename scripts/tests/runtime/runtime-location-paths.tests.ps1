...
try {
    ...
    Assert-True (Test-Path -LiteralPath (Join-Path (Join-Path $codexRuntimeRoot 'bin') (Get-RuntimeBinaryFileName)) -PathType Leaf) 'bootstrap must project the managed ntk runtime binary into the override Codex path.'
