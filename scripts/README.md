...
| `runtime/render-github-instruction-surfaces.ps1` | Regenerates the rendered GitHub instruction/runtime surface from `definitions/providers/github/{root,agents,chatmodes,prompts,hooks}/` plus `definitions/shared/{instructions,templates}/` into `.github/`. GitHub-native issue templates, PR templates, and dependency automation config remain authored directly in `.github/`. | `pwsh -File scripts/runtime/render-github-instruction-surfaces.ps1 -RepoRoot .` |
