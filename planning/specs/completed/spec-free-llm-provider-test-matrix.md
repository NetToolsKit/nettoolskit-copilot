...
- Planning Path: `planning/completed/plan-free-llm-provider-test-matrix.md`
...
- Freeze the provider-family matrix in one embedded catalog so report surfaces can explain quota and compatibility decisions without rediscovering provider assumptions at runtime.
...
- Initial implementation proof now exists for provider-mode boundaries through the built-in profile catalog and `ntk ai profiles` inspection surfaces, and the read-only usage-report enrichment proves the matrix can surface quota/fallback context without altering runtime selection.