...
- [2026-03-27 18:58] The first cross-crate Wave 3 consolidation is now live too: `crates/commands/validation/agent_orchestration` reuses the typed `crates/orchestrator` pipeline contract instead of maintaining a second local manifest model, so validation, permissions, and skill-alignment checks now consume the same stage/dispatch/runtime contract that future parity-harness work will target.
...
55. [2026-03-27 18:58] With validation now consuming the same typed orchestrator pipeline contract, the remaining Wave 3 parity-harness work can focus on behavior and evidence artifacts instead of schema drift; the next slice should port the `approval-approved-test` golden path first, then the staged `run-test` closeout success path.
56. [2026-03-27 19:25] With the native `approval-approved-test` parity harness now live in `crates/orchestrator/tests/execution/pipeline_parity`, the first Rust-owned success path covers real `run-agent-pipeline` plus `replay-agent-run` execution with deterministic fake Codex dispatch and temporary validation-green fixtures; the remaining Wave 3 parity backlog should now move to the staged `run-test` closeout success path before opening `resume-agent-pipeline` and `evaluate-agent-pipeline`.
57. [2026-03-27 19:42] With the staged `run-test` closeout success parity harness now live in `crates/orchestrator/tests/execution/pipeline_parity`, the Rust-owned Wave 3 coverage includes the real PowerShell stage chain from `intake` through `closeout`, idempotent `spec/plan` replays, README/CHANGELOG mutation evidence, and completed plan/spec timestamp preservation; the remaining parity backlog is now limited to `resume-agent-pipeline` and `evaluate-agent-pipeline`.
58. [2026-03-27 19:52] With the native `evaluate-agent-pipeline` parity coverage now live in `crates/orchestrator/tests/execution/pipeline_parity`, the Wave 3 harness also validates the repository-owned eval fixture scorecard against the real PowerShell entrypoint without dispatch-side mutation; the remaining parity backlog is now limited to `resume-agent-pipeline`.
59. [2026-03-27 20:01] With the native `resume-agent-pipeline` parity harness now live in `crates/orchestrator/tests/execution/pipeline_parity`, the Wave 3 harness also covers checkpoint-based continuation from a partial run stopped at `validate` through resumed execution at `review`; Task 7 is complete and the next migration work should move to Task 8 wrapper cutover sequencing.
...
- Updated: `2026-03-27 18:58` — reused the typed orchestrator pipeline contract inside `crates/commands/validation/agent_orchestration`, removed the duplicate local pipeline manifest model from the validation boundary, and narrowed the next Wave 3 slice to the first native parity-harness golden path (`approval-approved-test`, followed by the staged `run-test` closeout success path).
- Updated: `2026-03-27 19:25` — implemented the native `approval-approved-test` parity harness in `crates/orchestrator/tests/execution/pipeline_parity`, verified real `run-agent-pipeline` plus `replay-agent-run` success with deterministic fake Codex dispatch and temporary validation-green fixtures, and reduced the remaining Wave 3 parity backlog to `run-test` closeout success followed by `resume-agent-pipeline` and `evaluate-agent-pipeline`.
- Updated: `2026-03-27 19:42` — implemented the staged `run-test` closeout success parity harness in `crates/orchestrator/tests/execution/pipeline_parity`, verified the real PowerShell stage chain from `intake` through `closeout` with idempotent `spec/plan` replays plus README/CHANGELOG and completed-plan/spec evidence, and reduced the remaining Wave 3 parity backlog to `resume-agent-pipeline` and `evaluate-agent-pipeline`.
- Updated: `2026-03-27 19:52` — implemented the native `evaluate-agent-pipeline` parity coverage in `crates/orchestrator/tests/execution/pipeline_parity`, verified the repository-owned eval fixture scorecard against the real PowerShell entrypoint, and reduced the remaining Wave 3 parity backlog to `resume-agent-pipeline`.
- Updated: `2026-03-27 20:01` — implemented the native `resume-agent-pipeline` parity harness in `crates/orchestrator/tests/execution/pipeline_parity`, verified checkpoint capture through `validate` plus resumed execution at `review`, and closed the full Task 7 Wave 3 control-plane/parity block before wrapper cutover work in Task 8.
