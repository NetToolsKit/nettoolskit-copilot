...
| `scripts/maintenance/*.ps1` | 5 | `crates/commands/runtime` | command tests plus filesystem fixture validation for mutation-heavy flows | `evidence gap remains` | `clean-build-artifacts` and `trim-trailing-blank-lines` now run natively with direct tests, but `fix-region-spacing`, `fix-version-ranges`, and `generate-http-from-openapi` still lack a full native replacement record. |
