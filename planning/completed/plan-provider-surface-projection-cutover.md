...
- Spec Path: `planning/specs/completed/spec-provider-surface-projection-cutover.md`
- SDD Baseline: `planning/specs/active/spec-spec-driven-development-operating-model.md`
- Completion Note: canonical provider consumers, runtime renderers, validation fixtures, governance catalogs, projected templates, chatmodes, prompts, and generated `.github/.codex/.claude` surfaces now resolve from `definitions/*`; remaining instruction-architecture warnings are limited to AGENTS/global-instructions character budgets, not path drift.
- Dependencies:
...
- Result:
  - completed through canonical inventory of `.github/.codex/.claude` authored-vs-generated boundaries, with validators updated to treat `definitions/*` as the primary source of truth.
...
- Result:
  - completed by normalizing renderer catalogs, governance policy roots, and projected relative-link rewrites so GitHub/Codex/Claude outputs can be regenerated from canonical provider inputs.
...
- Result:
  - completed with full provider-surface re-render, canonical GitHub policy/governance projections, nested template projection under `.github/templates/*`, and reduced validation noise to non-projection warnings only.
...
- Result:
  - completed at the authored/runtime boundary: legacy mirrors remain available only as generated compatibility outputs, while provider consumers, prompts, test fixtures, and validation code no longer treat them as authored roots.
...
- `cargo run -q -p nettoolskit-cli -- runtime render-provider-surfaces --enable-codex-runtime --enable-claude-runtime`
- `cargo run -q -p nettoolskit-cli -- validation instructions --warning-only true`
- `cargo run -q -p nettoolskit-cli -- validation authoritative-source-policy --warning-only true`
- `cargo run -q -p nettoolskit-cli -- validation instruction-architecture --warning-only true`
- `cargo run -q -p nettoolskit-cli -- validation dotnet-standards`
- `cargo run -q -p nettoolskit-cli -- validation template-standards --warning-only true`
- `cargo run -q -p nettoolskit-cli -- validation planning-structure --warning-only true`
